...
No honest system can guarantee that every possible task is flawless. The correct target is a **general execution substrate** with explicit capability discovery, domain playbooks, reversible planning, finite execution budgets, confirmation gates for consequential actions, durable recovery, and benchmark-backed quality thresholds.
...
| Sports | Dedicated Sports mode/persona with source-aware match, scouting, player, league, event, and sports-business guidance plus deterministic team-summary, player-comparison, and validation tools | No licensed live sports-data feed, tactical video model, injury prediction, or guaranteed forecast; supplied data and source quality remain decisive |
| Fitness | Dedicated Fitness mode/persona with conservative planning, tracking, recovery reflection, and deterministic bounded plan/progress/validation tools | Not medical or clinical advice; no diagnosis, treatment, individualized medical clearance, or guaranteed health outcome |
...
| Data analysis | Data specialist plus deterministic CSV/TSV/JSON/JSONL profiling, validation, grouped summaries, correlation, linear regression, and bounded deterministic bar/flow visualizations with terminal and SVG artifacts | No advanced statistical inference, causal analysis, experiment significance testing, broad chart grammar, or automated leakage detection |
...
| Automation | Shell, browser bridge, task orchestration, autonomy profiles, durable schedules, a single-flight daemon, goal graphs, finite governed-tool budgets, and explicit `--unattended` execution exist | No hosted event ingress or provider-native background-interaction adapter; deployment still requires an online host |
| Specialist modes | Sports and Fitness are true ambient modes with mode-specific top-level and sub-agent prompts; both are also routable personas with capability contracts | No domain-specific live data connectors or clinical/competition-grade decision guarantees |
...
| Strategic intelligence | Battmann mode has a workspace-local transactional SQLite system of record, temporally gated immutable forecast revisions, live/backtest separation, proper-score calibration and chronological benchmarks, claim excerpts and append-only reviews, versioned ontology objects/links, scenarios, decisions/outcomes, Markdown/JSON/printable-HTML reports, Exa/Serper/Brave research, browser observation, and mode-aware recurring runs | A supported review is still analyst judgment rather than truth proof; SQLite and metadata classifications are not multi-tenant enterprise controls; superiority requires a sufficiently large independently resolved live set, credible external baselines, independent replication, and licensed/configured domain feeds |
...
| Completion truth | Completion assessor distinguishes verified, partial, blocked, failed, and aborted runs with evidence, blockers, confidence, and next actions; completion requires all step/delegation nodes and durable actions to be resolved; budget exhaustion is recorded and cannot verify a run | Subjective acceptance and external-world outcomes still need domain-specific postconditions or human review |
| Long-running autonomy | Durable goal graph, schedules with optional action-count and wall-clock budgets, single-flight daemon, leases, checkpoints, parent/child task sessions, operator pause/cancel propagation, run receipts, repair attempts, profiles, structured completion states, and resume support exist | The local daemon is not a hosted 24/7 service; external event delivery and provider-native background execution are not included |
| Safe unattended action | Unattended mode can continue bounded read-only, reversible, and idempotent work with command/browser readiness checks, exit/artifact/UI postconditions, retries, and human-review escalation; critical actions remain blocked even if a callback is supplied; fleet and standalone workers verify declared commands; Sports/Fitness deterministic analysis is safe to run locally | Critical external effects remain approval/takeover gated; safe classification cannot prove a third-party system’s final state, sports truth, or health suitability |
...
2. **Plan-and-execute contract.** Every non-trivial task should produce a goal, assumptions, dependencies, execution waves, external side effects, acceptance criteria, and a recovery policy before execution. The proposal parser bounds a plan to 32 work steps and 16 verification commands, directing larger goals toward smaller resumable runs. The existing proposal/goal-graph system carries these fields through parent sessions, fleet workers, standalone delegated tasks, receipts, and verification evidence; the remaining gap is domain-aware output beyond the current supported workflows.
...
8. **Unattended control plane.** Recurring work may be persisted as a schedule and executed by a single-flight daemon. Each action receives an idempotency key, bounded retry policy, precondition checks, postcondition/evidence checks, and a failure disposition. Missing configuration, stale leases, non-zero commands, unmet expectations, exhausted action budgets, ambiguous outcomes, and critical actions must surface as retryable or human-review states rather than being silently retried or reported as success.
The daemon is a local process; continuous operation requires an online machine or hosted worker.

9. **End-to-end delivery state.** Each autonomous run owns a parent task session. Proposal metadata is copied into that session, wave and variant progress is updated during execution, worker verification is recorded with evidence, operator pause/cancel signals abort the same controller used by all phases, and the final session points to its receipt, blockers, and next action. Per-run and scheduled runs may use finite governed-tool budgets; a blocked budget request is journaled and written to the receipt. A requested `completed` outcome is only returned when the durable graph accepts it and no budget block occurred; otherwise the run is persisted as `needs-attention`.
