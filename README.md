...
Schedules are stored atomically in `.elia/schedules.json`. Each claimed run has a lease, a bounded wall-clock budget, a persisted outcome, and a recovery path if the worker stops. The daemon invokes `elia auto` with unattended governance, but unattended does **not** mean unrestricted: read-only, reversible, and idempotent work may proceed; non-zero command results, missing browser transports, unmet postconditions, stale leases, and ambiguous outcomes become retryable or human-review states. Concurrent daemons use a reclaimable store lock and skip already-claimed work instead of terminating. Sending messages, publishing, purchasing, deleting, changing account state, production mutations, authentication, CAPTCHA, and payment steps remain blocked or require exact user approval/takeover.
