...
export const browserTool: Tool = {
  ...
  async execute(input) {
    ...
    if (SENSITIVE_WORDS.test(sideEffectText) && request.action !== 'status' && request.action !== 'snapshot' && request.action !== 'extract' && !request.confirmed) {
      return `Confirmation required before browser action "${request.action}" because it may cause an external side effect. Ask the user to approve the exact action, then retry with confirmed=true.`
    }
