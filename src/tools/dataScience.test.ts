...
function executeDataScience(input: Record<string, unknown>) {
  return withAgentIdentity({ name: 'test', role: 'lead', cwd: testDir }, () => dataScienceTool.execute(input))
}

test('data science profile reports schema, missingness, duplicates, and numeric summaries', async () => {
  const result = JSON.parse(await executeDataScience({ action: 'profile', path: datasetPath })) as { dataset: { rows: number; columns: string[] }; columns: Array<{ name: string; inferredType: string; missing: number; unique: number }>; duplicateRows: number; qualityStatus: string }
...
test('data science validation catches missing, non-numeric, and duplicate-key violations', async () => {
  const result = JSON.parse(await executeDataScience({ action: 'validate', path: datasetPath, requiredColumns: ['id', 'region', 'revenue'], uniqueColumns: ['id'], notNullColumns: ['region'], numericColumns: ['revenue'], minRows: 5 })) as { status: string; issues: Array<{ rule: string; column?: string }> }
...
test('data science grouped summaries and correlation are deterministic', async () => {
  const grouped = JSON.parse(await executeDataScience({ action: 'group_summary', path: datasetPath, groupBy: 'region', measure: 'revenue' })) as { groups: Array<{ group: string; sum: number; count: number; mean: number | null; median: number | null; min: number | null; max: number | null }> }
  expect(grouped.groups[0]).toEqual({ group: 'East', count: 2, sum: 180, mean: 90, median: 90, min: 80, max: 100 })
  const correlated = JSON.parse(await executeDataScience({ action: 'correlation', path: datasetPath, xColumn: 'id', yColumn: 'revenue' })) as { completePairs: number; pearsonR: number }
  expect(correlated.completePairs).toBe(4)
  expect(correlated.pearsonR).toBeCloseTo(-0.392232, 5)
  const regression = JSON.parse(await executeDataScience({ action: 'linear_regression', path: datasetPath, xColumn: 'id', yColumn: 'revenue' })) as { completePairs: number; coefficients: { slope: number; intercept: number }; metrics: { rSquared: number }; interpretation: string }
