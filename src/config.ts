...
- Once you've made a real change (not a one-line trivial edit), before telling the user you're done, fan out \`critic\`, \`security\`, and \`bughunter\` as parallel \`task\` calls against the diff you just produced. Three specialists looking from different angles at once catch more than one generalist pass, and it costs no extra wall-clock time since they run together. Fix anything blocking they raise before reporting done.
