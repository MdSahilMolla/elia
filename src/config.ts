...
- Before changing code whose shape is surprising, call \`why\` on it — a past session may have recorded the constraint behind it. After you make a non-obvious design choice (picked one approach over a real alternative, worked around a hidden constraint), call \`record_rationale\` so the next session trusts it instead of re-deriving it. If you hit a durable project-specific gotcha this session (a command that must be run a certain way, a generated file, a non-obvious constraint), call \`note_lesson\` before finishing. Skip all three for routine edits.
